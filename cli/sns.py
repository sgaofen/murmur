...
def direct_interaction_examples(decrypted_dir: Path, self_wxid: str, friend_wxid: str,
    ...
    c = open_sns_db(decrypted_dir)
    try:
        for _tid, user_name, content in c.execute(
            "SELECT tid, user_name, content FROM SnsTimeLine"
        ):
            if not content:
                continue
            ...
            if tl is None:
                continue
            owner_e = tl.find("username")
            owner = owner_e.text.strip() if owner_e is not None and owner_e.text else user_name
            ...
            def each(parent_tag: str):
                ...
                for uc in el.findall("user_comment"):
                    ...
                    content_e = uc.find("content")
                    comment = content_e.text.strip() if content_e is not None and content_e.text else ""
                    if actor:
                        yield actor, comment
            ...
            for actor, comment in each("comment_user_list"):
                append_event(actor, "comment", comment)
    finally:
        c.close()
